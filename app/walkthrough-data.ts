...
export const walkthroughStages: WalkthroughStage[] = [
  ...
  },
  {
    ...
    summary: "Luna proposed cancellation and renewal logic, plus three shared terms. Only after approval did it create canonical files and measured scope.", state: "Current behavior mapped",
  ...
  },
  {
    ...
    summary: "Luna added actor and plan behavior, scheduled annual cancellation, and cancellation at renewal. The suite grew from three to ten cases.", state: "Paired implementation",
    ...
    panels: {
      ...
      terminal: { filename: "repository terminal", label: "paired", lines: [plain("$ npm test"), add("tests 10 · pass 10 · fail 0"), plain("$ npm exec --offline -- sourcetwin check --base HEAD"), plain("ST604 · logic and mapped implementation changed together"), add("Coverage: code 2 / 2 · tests 10 / 10 direct")] },
    },
  },
  {
    id: "refine", label: "Refine + review", title: "A late production requirement stays traceable.",
    summary: "The user required repeated annual cancellation to return the exact same object and never move the date. Luna changed prose, code, and one test, then reviewed remaining gaps.", state: "Final reviewed state",
    ...
    panels: {
      ...
      terminal: { filename: "repository terminal", label: "final review", lines: [plain("$ npm exec --offline -- sourcetwin check --base HEAD"), add("Source Twin is valid."), add("Application tests: 11 / 11"), add("Code entities: 2 / 2 direct"), add("Test entities: 11 / 11 direct"), muted("Remaining gaps: invalid input, date edges, external billing effects.")] },
    },
  },
];
